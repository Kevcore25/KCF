...
def giveweapon(Item, Name, Description, DMG, CR, CD, ATKSPD, Status, Fire, Ice, Water, Electric, Nature):
    run(f'give @s {Item}[attribute_modifiers=[{{type:attack_damage,amount:-0.9,slot:mainhand,operation:add_value,id:"251"}},{{type:attack_speed,amount:-4,slot:mainhand,operation:add_value,id:"252"}},{{type:attack_speed,amount:{ATKSPD},slot:mainhand,operation:add_value,id:"253"}}],custom_name=[{{"text":"{Name}","italic":false}}],lore=[[{{"text":"{Description}","italic":false,"color":"gray"}}],"",[{{"text":"Base DMG: {DMG}","italic":false,"color":"gray"}}],[{{"text":"Base Status: {Status}%","italic":false,"color":"gray"}}],[{{"text":"Base Critical Chance: {CR}%","italic":false,"color":"gray"}}],[{{"text":"Base Critical Damage: {CD}%","italic":false,"color":"gray"}}],[{{"text":"Base Attack Speed: {ATKSPD}/s","italic":false,"color":"gray"}}]],custom_data={{DMG: {DMG}, CR: {CR}, CD: {CD}, ATKSPD: {ATKSPD}, Status: {Status}, Fire: {Fire}, Ice: {Ice}, Water: {Water}, Electric: {Electric}, Nature: {Nature}}}]')
...
def ww():                   
    execute('as @e[type=!player,distance=..16,tag=!ww] at @s facing entity @n[tag=ww] feet', run('tp @s ^ ^ ^0.05'))
    execute('as @e[type=!player,distance=..12,tag=!ww] at @s facing entity @n[tag=ww] feet', run('tp @s ^ ^ ^0.05'))
    execute('as @e[type=!player,distance=..10,tag=!ww] at @s facing entity @n[tag=ww] feet', run('tp @s ^ ^ ^0.05'))
    execute('as @e[type=!player,distance=..8,tag=!ww] at @s facing entity @n[tag=ww] feet',  run('tp @s ^ ^ ^0.05'))
    execute('as @e[type=!player,distance=..6,tag=!ww] at @s facing entity @n[tag=ww] feet',  run('tp @s ^ ^ ^0.05'))
    execute('as @e[type=!player,distance=..5,tag=!ww] at @s facing entity @n[tag=ww] feet',  run('tp @s ^ ^ ^0.05'))
    execute('as @e[type=!player,distance=..4,tag=!ww] at @s facing entity @n[tag=ww] feet',  run('tp @s ^ ^ ^0.05'))
    execute('as @e[type=!player,distance=..2,tag=!ww] at @s facing entity @n[tag=ww] feet',  run('tp @s ^ ^ ^0.05'))
    self.time += 1

    execute('as @e[type=!player,distance=..5,tag=!ww]', run('data modify entity @s Gravity set value 0b'))
    execute('as @e[type=!player,distance=5.1..10,tag=!ww]', run('data modify entity @s Gravity set value 1b'))


    if self.time > 10000:
        execute('as @e[type=!player,distance=..5,tag=!ww]', run('data modify entity @s Gravity set value 1b'))
        kill(self)

    run('particle minecraft:glow_squid_ink ~ ~ ~ 1 1 1 0 3')

def summonww():
    summon(marker, '~ ~ ~', {'Tags': '[ww]'})
